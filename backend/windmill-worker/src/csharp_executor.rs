...
use crate::{
    ...
    }, handle_child::handle_child, AuthedClientBackgroundTask, CSHARP_CACHE_DIR, DISABLE_NSJAIL, DISABLE_NUSER, DOTNET_PATH, HOME_ENV, NSJAIL_PATH, PATH_ENV, RUST_CACHE_DIR, TZ_ENV
};
...
fn gen_cs_proj(code: &str, job_dir: &str) -> anyhow::Result<()> {
    write_file(job_dir, "Main.csproj", r#"<Project Sdk="Microsoft.NET.Sdk">
...
"#)?;
    ...
    write_file(job_dir, "Program.cs", r#"
using System;
using System.IO;
...
class Program
{
    ...
    {
        string res = Script.moin("Arakis");
        WriteResultToJson(res);
    }

    static void WriteResultToJson(string result)
    {
...
async fn build_cs_proj(
    ...
    .await
    {
        Err(e) => {
            let em = format!(
                "could not save {job_dir}/Main to C# cache: {e:?}",
            );
            tracing::error!(em);
            Ok(em)
        }
